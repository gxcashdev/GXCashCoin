...
// Copyright (c) 2011-2012 GXCash Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #GXCashTEST0\r");
            Send(hSocket, "WHO #GXCashTEST0\r");
        } else {
            // randomly join #GXCash00-#GXCash99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #GXCash%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #GXCash%02d\r", channel_number).c_str());
        }
