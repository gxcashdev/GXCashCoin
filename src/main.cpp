...
uint256 hashGenesisBlock("0x98f14b7c6ded976b181f195e5a116e0b265173aad0b019c8839e06649efd64ae");
...
const string strMessageMagic = "GXCash Signed Message:\n";
...
{
    // GXCash: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    if (pblock->nTime - pindexLast->nTime > nTargetSpacing*1200)
        return bnProofOfWorkLimit.GetCompact();

	static const int64	BlocksTargetSpacing			= 2 * 60;
	unsigned int		TimeDaySeconds				= 60 * 60 * 24;
	int64				PastSecondsMin				= TimeDaySeconds * 2 * 0.1;
	int64				PastSecondsMax				= TimeDaySeconds * 2 * 2.8;
...
{
    // GXCash: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "G genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xa4, 0xc4, 0xaf, 0xe3 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GXCashMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GXCashMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GXCashMiner(CWallet *pwallet)
{
    printf("GXCashMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("GXCash-miner");
    ...
    try { loop {
        ...
        printf("Running GXCashMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GXCashMiner, pwallet));
}
